...
function SentReqCard(props) {
	...
	const cancel = (id) => {
		...
		axios
			.put(cancelURL, "", { headers: { Authorization: `Bearer ${token}` } })
			.then(() => {
				updateRequests()
				toast.info(
					`Cancelled request to ${props.request.to.name_first} ${props.request.to.name_last}`
				)
			})
	}
			...
			<Card.Content>
				...
				<Container>
					<Link to={`/users/${props.request.to._id}`}>
						<Heading
							size={4}
						>{`${props.request.to.name_first} ${props.request.to.name_last}`}</Heading>
					</Link>
				</Container>
				<Block></Block>
				<Content>
