...
function Comment(props) {
	...
	useEffect(() => {
		if (alreadyLiked == -1) {
			setLike(
				<Button size='small' color='success' onClick={likeFn}>
					<FaThumbsUp />
					&nbsp;Like{" "}
				</Button>
			)
			...
			setLike(
				<Button size='small' color='warning' onClick={unlikeFn}>
					<FaThumbsDown />
					&nbsp;Unlike{" "}
				</Button>
			)
	...
	const likeFn = () => {
		setLike(
			<Button size='small' color='success' loading>
				Liking
			</Button>
		)
		axios
	...
	const unlikeFn = () => {
		setLike(
			<Button size='small' color='warning' loading>
				Unliking
			</Button>
		)
		axios
				...
				<Card.Content display='flex' justifyContent='space-between'>
					<Media>
						<Media.Item align='left'>
							<Image
								size={32}
								src={`${process.env.REACT_APP_API_URL}/file/${comment.author.profile_pic}`}
							/>
						</Media.Item>
